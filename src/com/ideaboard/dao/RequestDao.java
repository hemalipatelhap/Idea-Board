...
public class RequestDao {
	...
	public int createRequests(Request ideaRequest) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into requests(ideaid,netid,status) values(?,?,?)");
			
			
			ps.setInt(1, ideaRequest.getIdeaId());
			ps.setString(2, ideaRequest.getNetId());
			ps.setString(3, ideaRequest.getStatus());
